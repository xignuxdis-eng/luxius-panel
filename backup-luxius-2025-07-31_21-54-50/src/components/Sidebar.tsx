    ...
    <aside className="w-64 bg-white shadow-lg h-screen sticky top-0 flex flex-col border-r border-gray-200">
      ...
      <nav className="flex-1 space-y-1 p-4">
        {roleLinks.map(({ to, label, icon: Icon }) => {
          ...
          if (to === "/dashboard/impresor") {
            // Para la página principal del impresor, solo activa si es exactamente esa ruta
            isActive = location.pathname === "/dashboard/impresor";
          } else {
            // Para otras rutas, verifica si la ruta actual comienza con el enlace
