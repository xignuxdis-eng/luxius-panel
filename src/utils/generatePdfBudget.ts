...
function gatherArtworkFiles(order: Order): ArtworkFile[] {
    const rawArtworkFiles: ArtworkFile[] = []
    ...
    if (order.archivos && order.archivos.length > 0) {
        order.archivos.forEach((file, index) => {
            ...
            rawArtworkFiles.push({ url: resolveMediaUrl(file), name: orig })
        })
    } else if (meta?.thumbnailUrl) {
        rawArtworkFiles.push({ url: resolveMediaUrl(meta.thumbnailUrl), name: order.archivosOriginales?.[0] || 'Archivo cargado' })
    }

    return rawArtworkFiles
}
...
function buildOrderPageHtml(order: Order, mode: BudgetPdfMode, artworkItems: ArtworkFile[]): string {
    const isSimplified = mode === 'simplificado'
    ...
    const badgeBg = isPrintedOrFinished ? '#15803d' : '#2563eb'
    ...
    const bottomBlocks = isSimplified
        ? `
    ...
    return `
        <div class="a4-page">
            <div>
                <div class="header-brand">
                    ...
                    <div class="brand-right">
                        <div class="badge-doc-type" style="background: ${badgeBg};">${docBadge}</div>
                ...
                <div class="artwork-and-items" style="grid-template-columns: ${previewImgUrl ? '150px 1fr' : '1fr'};">
                    ${previewImgUrl ? `
            ...
            <div class="fine-print">
                XignuX Servicios Gráficos e Impresión Digital Profesional · José V. Cardozo 912, Córdoba · Tel: 3517897667/3517717071 · ${isSimplified ? 'Documento simplificado sin valores monetarios.' : 'Presupuesto sujeto a confirmación técnica de archivos.'}
            </div>
        </div>
    `
}

async function openBudgetPdf(orders: Order[], mode: BudgetPdfMode, windowTitle: string, pdfFilename: string) {
    ...
    // Optimizar miniaturas de todas las órdenes con deduplicación por URL
    const filesByOrder = orders.map(gatherArtworkFiles)
    const uniqueUrls = Array.from(new Set(filesByOrder.flat().map(f => f.url)))
    ...
    const urlMap = new Map(uniqueUrls.map((u, i) => [u, optimizedUrls[i] || u]))

    const pages = orders.map((order, i) => {
        const artworkItems = filesByOrder[i].map(f => ({ url: urlMap.get(f.url) || f.url, name: f.name }))
        return buildOrderPageHtml(order, mode, artworkItems)
    })
    ...
    const htmlContent = `
        ...
        <body>
            <div class="no-print-bar">
                <span><strong>${windowTitle}</strong> — XignuX Gráfica</span>
...
export async function generatePdfBudget(order: Order, options: BudgetPdfOptions = {}) {
    const mode = options.mode || 'detallado'
    const otDisplay = order.ot || `OT-${strId(order.id)}`
    const docTitle = ['impreso', 'post', 'completo', 'entregado', 'finalizado'].includes(order.status)
        ? 'DETALLE DE IMPRESIÓN'
        : 'PRESUPUESTO COMERCIAL'
    const pdfFilename = `${docTitle.replace(/\s+/g, '_')}_XignuX_${otDisplay}.pdf`
    await openBudgetPdf([order], mode, `${docTitle} (${otDisplay})`, pdfFilename)
}
